...
top20 = fread('../Top20.csv')
setnames(top20, 'Institution Name (RSSD ID)', 'Institution Name')
# Note: Assets in thousands of dollars
top20[, `12/31/2016 Total Assets`:= as.numeric(
  gsub('[^0-9]','',`12/31/2016 Total Assets`))/1e6]
top20[, `Institution Name`:= gsub(' \\(\\d+\\)', '', `Institution Name`)]
top20[, `Institution Name`:= gsub('ASSOCIATION', 'ASSOC.', `Institution Name`)]
top20[, `Institution Name`:= factor(`Institution Name`, `Institution Name`)]

ggplot(top20, aes(y=`12/31/2016 Total Assets`, x=`Institution Name`)) +
  geom_bar(stat='identity', fill='royalblue') +
  labs(x='', y='12/31/2016 Total Assets (billions)') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.margin = unit(c(.2,1.4,0,1.4), 'cm'))
```
